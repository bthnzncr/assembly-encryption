...
space: .asciiz " "    # Define newline symbol

.text
  read:
    la $a0, input      # Load the address of the input buffer
    li $a1, 128        # We are prepared to handle 128 characters of input
...
LastCharacterCheck:
    lbu $t1, 0($k1)
    beq $t1, $t5, updateLastCharacter
    addi $k1, $k1, 1
    j LastCharacterCheck
...
process:
	...
	# Shift and OR each byte into the final word
      sll $t0, $t0, 24  # shift left logical by 24 bits
	or $s0, $s0, $t0

	sll $t1, $t1, 16  # shift left logical by 16 bits
	or $s0, $s0, $t1

  sll $t2, $t2, 8   # shift left logical by 8 bits
    or $s0, $s0, $t2	
    or $s0, $s0, $t3
...
move $a0, $t9    
   ...
   move $a0, $t9 
    ...
    li $s0, 0		# Shift and OR each byte into the final word
    sll $t0, $t0, 24  # shift left logical by 24 bits
    or $s0, $s0, $t0
...
KeyWhiteningFirstIteration:
	la $t7, message
	lw $t0, 0($t7)		# m[0]
	
	lw $t1, 0($a3)		# key[0]
	
	xor $t1, $t1, $t0 	# key [0] ^ m[0]
	sw $t1, 0($a0) 		# s[0] = key[0] ^ m[0]
	
	lw $t0, 4($t7)		# m[1]
	lw $t1, 4($a3)		# key[1]
	
	xor $t1, $t1, $t0 	# m[1] ^ key[1]
	sw $t1, 4($a0) 		# s[1] = m[1] ^ key[1]
	
	lw $t0, 8($t7)		# m[2]
	lw $t1, 8($a3)		# key[2]
	
	xor $t1, $t1, $t0 	# m[2] ^ key[2]
	sw $t1, 8($a0) 		# s[2] = m[2] ^ key[2]
	
	lw $t0, 12($t7)		# m[3]
	lw $t1, 12($a3)		# key[3]
	
	xor $t1, $t1, $t0 	# m[3] ^ key[3]
	sw $t1, 12($a0) 		# s[3] = m[3] ^ key[3]
...
Round:
	lw $t0, 0($a2)	 	# Load the s[0]	
	lw $t1, 4($a2) 		# Load the s[1]
	lw $t2, 8($a2) 		# Load the s[2]
	lw $t3, 12($a2) 		# Load the s[3]
